...
export default function ConnectionGraph() {
  ...
  const getNodeObject = useCallback((node: any) => {
    const isHighlighted = !focusedNode || highlightNodes.has(node);
    const baseColor = node.flag
      ? FLAG_COLORS[node.flag]
      : TYPE_COLORS[node.type];
    const color = isHighlighted ? baseColor : "#111122";

    const group = new THREE.Group();
    let geometry;
    if (node.type === "fund_manager") {
      geometry = new THREE.SphereGeometry(
        node.aum ? Math.max(4, node.aum * 0.8) : 6,
        32, 32
      );
    ...
    const material = new THREE.MeshLambertMaterial({
      color,
      transparent: true,
      opacity: isHighlighted ? 0.92 : 0.12,
    ...
    if (node.flag === "red" && isHighlighted) {
      const pulse = () => {
        const t = Date.now() / 500;
        material.emissiveIntensity = 0.6 + Math.sin(t) * 0.4;
  ...
  const getNodeLabel = useCallback((node: any) => {
    ...
    return `<div style="background:rgba(10,10,30,0.95);
      padding:10px 14px;border-radius:6px;
      border:1px solid ${color};color:#fff;
      font-family:Inter,sans-serif;
      pointer-events:none">${lines}</div>`;
  }, []);
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          color: "#C9A84C", fontWeight: 700, marginBottom: 8
        }}>
          INTERCONNECTION MAP
        </div>
        {[
        ...
        <div style={{
          ...
          marginTop: 8, paddingTop: 8, color: "#888"
        }}>
        ...
        <div style={{
          ...
          <div style={{ color: "#ccc", lineHeight: 1.8 }}>
            ...
            Risk Score: <span style={{
              color: focusedNode.riskScore > 50
                ? "#EF4444"
